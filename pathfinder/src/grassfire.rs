...
pub fn v1(area: &mut [u64], rows: &usize, cols: &usize, start:(usize, usize), end:(usize, usize)) -> Vec<(usize, usize)> {
	...
	find_path("Grassfire-V1", start, area, rows, cols)
}
...
pub fn v2(area: &mut [u64], rows: &usize, cols: &usize, start:(usize, usize), end:(usize, usize)) -> Vec<(usize, usize)> {
	...
	find_path("Grassfire-V2", start, area, rows, cols)
}
...
pub fn v3(area: &mut [u64], rows: &usize, cols: &usize, start:(usize, usize), end:(usize, usize)) -> Vec<(usize, usize)> {
	...
	find_path("Grassfire-V3", start, area, rows, cols)
}
...
pub fn find_path(algorithm: &str, start: (usize, usize), area: &mut [u64], rows: &usize, cols: &usize) -> Vec<(usize, usize)> {
	let benchmark = Instant::now();
	...
	loop {
		...
		let value = cell_value - 1;
		let mut check_neighbor = |field: Option<(usize, usize)>| {
			if let Some(fld) = field {
				if area[(fld.1 * rows) + fld.0] == value {
					row_num = fld.0;
					col_num = fld.1;
					return true;
				}
		...
		// Try the neighbors, direct connected (up/down/left/right) are preferred to the edges
		if !check_neighbor(col_prev) &&
