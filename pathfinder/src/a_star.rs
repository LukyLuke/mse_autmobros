...
pub fn v1(area: &mut [u64], rows: &usize, cols: &usize, start:(usize, usize), end:(usize, usize)) -> Vec<(usize, usize)> {
	...
	let mut last_fields: Vec<(usize, usize)> = vec![start];
	...
	// TODO: check later...
	area[(start.1 * rows) + start.0] = calculate_distance(end) as u64;
	...
	loop {
		...
		let min_path_estimation = last_fields.iter()
			.map(|field| {
				let fld = &calculated_fields[(field.1 * rows) + field.0];
				match fld {
					Some(f) => if f.processed { u64::MAX } else { f.estimate },
					None => u64::MAX
				}
			})
			.min_by(|a, b| a.cmp(b))
		...
		// Calculate all surrounding fields
		let mut next_fields: Vec<(usize, usize)> = vec![];
		last_fields.iter()
			.for_each(|field| {
				let current_field: &mut Field = calculated_fields[(field.1 * rows) + field.0].as_mut().unwrap();

				// Process only fields which are not blocked and have the minimal estimation value
				if !current_field.processed && current_field.estimate == min_path_estimation {
					changed = true;
		...
		last_fields.append(&mut next_fields);
